...
BYTE CGameLogic::GetCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
	...
	//三牌判断
	if (AnalyseResult.cbBlockCount[2]>0)         //至少一个三条
	{
		//连牌判断
		if (AnalyseResult.cbBlockCount[2]>1)           //两个三条以上
		{
			//变量定义
			BYTE cbCardData=AnalyseResult.cbCardData[2][0];          //第一个三条的子面数值
			BYTE cbFirstLogicValue=GetCardLogicValue(cbCardData);    //第一个三条的逻辑数值
			...
			for (BYTE i=1;i<AnalyseResult.cbBlockCount[2];i++)
			{
				...
				if (cbFirstLogicValue!=(GetCardLogicValue(cbCardData)+i)) return CT_ERROR;   //不构成飞机
			}
		}
		else if( cbCardCount == 3 ) return CT_THREE;                  //单个三条

		//牌形判断
		if (AnalyseResult.cbBlockCount[2]*3==cbCardCount) return CT_THREE_LINE;     //三条
		if (AnalyseResult.cbBlockCount[2]*4==cbCardCount) return CT_THREE_TAKE_ONE;   //三带一
		if ((AnalyseResult.cbBlockCount[2]*5==cbCardCount)&&(AnalyseResult.cbBlockCount[1]==AnalyseResult.cbBlockCount[2])) return CT_THREE_TAKE_TWO;  //三带对 或 飞机
	...
	if (AnalyseResult.cbBlockCount[1]>=3)
	{
		...
		//二连判断
		if ((AnalyseResult.cbBlockCount[1]*2)==cbCardCount) return CT_DOUBLE_LINE;      //连对
...
VOID CGameLogic::SortCardList(BYTE cbCardData[], BYTE cbCardCount, BYTE cbSortType)
{
	...
	if (cbSortType==ST_COUNT)
	{
		...
		{
			//拷贝扑克
			BYTE cbIndex=CountArray(AnalyseResult.cbBlockCount)-i-1;     //获得从四牌到单牌下标
...
VOID CGameLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	...
	do
	{
		...
		cbCardBuffer[cbRandCount++]=cbCardData[cbPosition];  //src随机一张牌给作缓冲的数组 下标作自增
...
bool CGameLogic::RemoveCardList(const BYTE cbRemoveCard[], BYTE cbRemoveCount, BYTE cbCardData[], BYTE cbCardCount)
{
	...
	for (BYTE i=0;i<cbRemoveCount;i++)
	{
		...
		{
			if (cbRemoveCard[i]==cbTempCardData[j])//如果源码有目标牌型
			{
				cbDeleteCount++;                   //计数
				cbTempCardData[j]=0;               //在缓冲数组中置零
				break;
			}
		}
	}
	...
	{
		if (cbTempCardData[i]!=0) cbCardData[cbCardPos++]=cbTempCardData[i];//cbCardData[cbCardPos]后面的没有清空置零
	}
...
VOID CGameLogic::AnalysebCardData(const BYTE cbCardData[], BYTE cbCardCount, tagAnalyseResult & AnalyseResult)
{
	...
	for (BYTE i=0;i<cbCardCount;i++)
	{
		...
		BYTE cbLogicValue = GetCardLogicValue(cbCardData[i]);/*获得逻辑上的数值*/

		//搜索同牌
		for (BYTE j=i+1;j<cbCardCount;j++)
		{
			//获取扑克
			if (GetCardLogicValue(cbCardData[j])!=cbLogicValue) break; /*找出后面的等值的card 当下一张不相同时退出内循环*/
								...
								tagSearchCardResult *pSearchCardResult )
{
	...
	BYTE cbReferLogicValue = cbReferCard==0?0:GetCardLogicValue(cbReferCard); //0
	BYTE cbBlockIndex = cbSameCardCount-1; //0:单牌
	do
	{
		for( BYTE i = 0; i < AnalyseResult.cbBlockCount[cbBlockIndex]; i++ )//以单牌个数为循环次数
		{
			BYTE cbIndex = (AnalyseResult.cbBlockCount[cbBlockIndex]-i-1)*(cbBlockIndex+1);//单牌数
